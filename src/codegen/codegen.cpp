...
std::string registerToString(X86Register reg)
{
  ...
  case X86Register::RDI:
    return "rdi";
  case X86Register::RBP:
    return "rbp";
  case X86Register::RSP:
    return "rsp";
  case X86Register::R8:
    return "r8";
  case X86Register::R9:
  ...
  case X86Register::R11:
    return "r11";
  case X86Register::R12:
    return "r12";
  case X86Register::R13:
    return "r13";
  case X86Register::R14:
    return "r14";
  case X86Register::R15:
    return "r15";
  // XMM registers (floating-point, System V AMD64 ABI)
  case X86Register::XMM0:
    return "xmm0";
  case X86Register::XMM1:
    return "xmm1";
  case X86Register::XMM2:
    return "xmm2";
  case X86Register::XMM3:
    return "xmm3";
  case X86Register::XMM4:
    return "xmm4";
  case X86Register::XMM5:
    return "xmm5";
  case X86Register::XMM6:
    return "xmm6";
  case X86Register::XMM7:
    return "xmm7";
  case X86Register::XMM8:
    return "xmm8";
  case X86Register::XMM9:
    return "xmm9";
  case X86Register::XMM10:
    return "xmm10";
  case X86Register::XMM11:
    return "xmm11";
  case X86Register::XMM12:
    return "xmm12";
  case X86Register::XMM13:
    return "xmm13";
  case X86Register::XMM14:
    return "xmm14";
  case X86Register::XMM15:
    return "xmm15";
  // Invalid/spilled
...
LinearScanAllocator::LinearScanAllocator() : nextSpillSlot(0)
{
  ...
  // (A production compiler would also use callee-saved with proper
  // save/restore)
...
void LinearScanAllocator::buildLiveIntervals(IRFunction *function)
{
  ...
  // Map SSA names to their live intervals (use names since operands contain
  // copies)
...
PeepholeOptimizer::PeepholeOptimizer() : optimizationEnabled(true) {}
...
std::string CodeGenerator::getOperandString(const IROperand &operand)
{
  if (operand.isConstant())
  {
    ...
    if (isStringLiteral)
    {
      ...
      // reference
      std::string label =
          const_cast<CodeGenerator *>(this)->addStringLiteral(constVal);
...
bool CodeGenerator::isCalleeSaved(X86Register reg) const
{
  ...
  return (reg == X86Register::RBX || reg == X86Register::R12 ||
          reg == X86Register::R13 || reg == X86Register::R14 ||
...
void CodeGenerator::emitArithmeticInst(const IRInstruction *inst)
{
  ...
  emitComment(result->getSSAName() + " = " + operands[0].toString() + " " +
              inst->toString().substr(0, 3) + " " + operands[1].toString());
...
void CodeGenerator::emitComparisonInst(const IRInstruction *inst)
{
  ...
  emitComment(result->getSSAName() + " = " + operands[0].toString() + " cmp " +
              operands[1].toString());

  // Compare instruction: AT&T syntax is "cmp src, dst" but semantics are dst -
  // src So cmpq $5, %rax compares rax with 5 (rax - 5)
  emit("cmpq " + src2 + ", " + src1);
  ...
  // R8->R8B, R9->R9B, R10->R10B, R11->R11B, R12->R12B, R13->R13B, R14->R14B,
  // R15->R15B
...
void CodeGenerator::emitLocationDirective(int line, int column)
{
  ...
  if (line == currentSourceLine)
    return; // Don't emit duplicate .loc for same line
...
void CodeGenerator::emitCallInst(const IRInstruction *inst)
{
  ...
  // - Floating-point arguments (first 8): XMM0, XMM1, XMM2, XMM3, XMM4, XMM5,
  // XMM6, XMM7
  ...
  int floatArgIndex = 0;
  (void)
      floatArgIndex; // Suppress unused warning until float args are implemented
...
void CodeGenerator::emitLoadInst(const IRInstruction *inst)
{
  ...
  emitComment("Load from memory: " + result->getSSAName() + " = *(" +
              operands[0].toString() + ")");
...
void CodeGenerator::emitStoreInst(const IRInstruction *inst)
{
  ...
  emitComment("Store to memory: *(" + operands[1].toString() +
              ") = " + operands[0].toString());
    ...
    const std::vector<std::unique_ptr<IRFunction>> &functions)
{
  ...
  // Generate code for each function (this populates externalSymbols and
  // dataSection)

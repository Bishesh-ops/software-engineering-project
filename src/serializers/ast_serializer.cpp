...
private:
  ...
  void addLocation(json &j, const ASTNode &node) {
    j["location"] = {{"file", node.getLocation().filename},
                     {"line", node.getLine()},
...
std::string
serializeAstToJson(const std::vector<std::unique_ptr<Declaration>> &ast) {
  json result;

...
void SemanticAnalyzer::visit(BinaryExpr &node)
{
    ...
    // For assignment operators, result type is the type of the left operand
    else if (op == "=" || op == "+=" || op == "-=" || op == "*=" || op == "/=" || op == "%=")
    {
        // Check that left operand is an lvalue
        if (!is_lvalue(node.getLeft()))
        {
            add_error("Assignment target must be an lvalue", node.getLocation());
            return;
        }

        // For simple assignment with implicit conversion, add a warning for narrowing
        if (op == "=" && !left_type->equals(*right_type))
        {
            if (right_type->canConvertTo(*left_type))
            {
                // Warn for potentially lossy conversions
                if (right_type->isFloatingPoint() && left_type->isIntegral())
                {
                    add_warning("Implicit conversion from '" + right_type->toString() +
                                    "' to '" + left_type->toString() +
                                    "' may lose precision",
                                node.getLocation());
                }
            }
        ...
        // Result type of assignment is the type of the left operand

...
bool SemanticAnalyzer::register_symbol(const Symbol &symbol,
                                       const SourceLocation &location) {
...
void SemanticAnalyzer::analyze_program(
...
void SemanticAnalyzer::register_builtin_functions() {
  ...
  register_builtin_function("fread", "int", {"void*", "int", "int", "void*"},
                            false);
  register_builtin_function("fwrite", "int", {"void*", "int", "int", "void*"},
                            false);
  ...
  register_builtin_function("strncpy", "char*", {"char*", "char*", "int"},
                            false);
  ...
  register_builtin_function("memcpy", "void*", {"void*", "void*", "int"},
                            false);
  register_builtin_function("memset", "void*", {"void*", "int", "int"}, false);
  register_builtin_function("memmove", "void*", {"void*", "void*", "int"},
                            false);
...
void SemanticAnalyzer::register_builtin_function(
    const std::string &name, const std::string &return_type,
    const std::vector<std::string> &param_types, bool is_variadic) {
  std::vector<std::shared_ptr<Type>> typed_params;
  for (const auto &pt : param_types) {
    typed_params.push_back(Type::fromString(pt));
  }

  Symbol func_symbol(Symbol::AsFunction, name, Type::fromString(return_type),
                     typed_params,
                     0 // Global scope
  );
...
std::shared_ptr<Type>
SemanticAnalyzer::get_expression_type(const Expression *expr) const {
...
void SemanticAnalyzer::set_expression_type(const Expression *expr,
...
bool SemanticAnalyzer::is_lvalue(const Expression *expr) const {
  ...
  // Everything else (literals, function calls, binary expressions, etc.) is not
  // an lvalue
  return false;
}

// Calculate Levenshtein distance between two strings (for finding similar
// identifiers)
int SemanticAnalyzer::levenshtein_distance(const std::string &s1,
                                           const std::string &s2) {
...
std::string
SemanticAnalyzer::find_similar_identifier(const std::string &name) const {
...
void SemanticAnalyzer::visit(VarDecl &node) {
  ...
  if (struct_it != struct_types_.end()) {
    ...
    if (node.getPointerLevel() > 0) {
      var_type = Type::makeStructPointer(type_name, node.getPointerLevel());
    }
  } else {
    ...
    if (node.getPointerLevel() > 0 && var_type) {
      var_type =
          Type::makePointer(var_type->getBaseType(), node.getPointerLevel());
    }
    // Handle arrays
  ...
  Symbol symbol(node.getName(), var_type,
...
void SemanticAnalyzer::visit(StructDecl &node) {
  ...
  for (const auto &field : node.getFields()) {
    if (field) {
      ...
      std::string type_str = field->getType();

      // Check if it's a struct type
      const std::string struct_prefix = "struct ";
      if (type_str.find(struct_prefix) == 0) {
        std::string struct_name = type_str.substr(struct_prefix.length());

        // Remove trailing pointers/whitespace for lookup
        std::string base_name = struct_name;
        while (!base_name.empty() &&
               (base_name.back() == '*' || std::isspace(base_name.back()))) {
          base_name.pop_back();
        }

        auto struct_it = struct_types_.find(base_name);
        if (struct_it != struct_types_.end()) {
          field_type = struct_it->second;
        } else {
          // Struct not found - fallback (likely unknown)
          // For self-referential structs (like linked lists), we might not have
          // it yet. We should create a placeholder or handle it. For now, let's
          // just create a struct type with that name but no members? Or just
          // fallback to unknown.
          field_type = Type::fromString(field->getType());
        }
      } else {
        field_type = Type::fromString(field->getType());
      }
      ...
      if (field->isPointer()) {
        if (field_type->isStruct()) {
          field_type = Type::makeStructPointer(field_type->getStructName(),
                                               field->getPointerLevel());
        } else {
          field_type = Type::makePointer(field_type->getBaseType(),
                                         field->getPointerLevel());
        }
      }
      // Handle array fields
  ...
  // Note: In C, struct tags are in a separate namespace, but we'll keep it
  // simple
}
...
void SemanticAnalyzer::visit(FunctionDecl &node) {
  ...
  for (const auto &param : node.getParameters()) {
    if (param) {
      ...
      std::string type_str = param->getType();
      int pointer_depth = 0;

      // Count and remove trailing asterisks
      ...
      // Trim whitespace
      while (!type_str.empty() && std::isspace(type_str.back())) {
        type_str.pop_back();
      }
      ...
      if (type_str.find(struct_prefix) == 0) {
        ...
        auto struct_it = struct_types_.find(struct_name);
        if (struct_it != struct_types_.end()) {
          // Found the struct type
          param_type = struct_it->second;

          // Apply pointer depth
          if (pointer_depth > 0) {
            param_type = Type::makeStructPointer(struct_name, pointer_depth);
          }
        } else {
          // Struct not found - fallback
          param_type = Type::fromString(param->getType());
        }
      } else {
        // Basic type
        param_type = Type::fromString(param->getType());
  ...
  Symbol func_symbol(Symbol::AsFunction, node.getName(),
                     Type::fromString(node.getReturnType()), param_types,
                     scope_manager_.get_current_scope_level());
  ...
  // Check for unused variables (parameters and locals) before exiting function
  // scope
...
void SemanticAnalyzer::visit(ParameterDecl &node) {
  // Parse the type string to handle pointers and structs
  std::string type_str = node.getType();
  int pointer_depth = 0;
  ...
  if (type_str.find(struct_prefix) == 0) {
    ...
    // Basic type
    param_type = Type::fromString(node.getType());
  }

  // Create symbol for parameter
  Symbol param_symbol(node.getName(), param_type,
                      scope_manager_.get_current_scope_level());
...
void SemanticAnalyzer::visit(IfStmt &node) {
  ...
  // Visit then branch (each branch creates its own scope if it's a compound
  // stmt)
      ...
      if (!current_function_return_type_->equals(*return_type)) {
        ...
        if (return_type->canConvertTo(*current_function_return_type_)) {
          ...
          if (return_type->isFloatingPoint() &&
              current_function_return_type_->isIntegral()) {
          ...
          add_error("Return type mismatch: expected '" +
                        current_function_return_type_->toString() + "', got '" +
...
void SemanticAnalyzer::visit(DeclStmt &node) {
  // Visit the wrapped declaration - this will register the variable in the
  // symbol table
...
void SemanticAnalyzer::visit(BinaryExpr &node) {
  ...
  if (!left_type->equals(*left_type_decayed)) {
    auto cast_node = std::make_unique<TypeCastExpr>(
        node.releaseLeft(), left_type_decayed->toString(),
  ...
  if (!right_type->equals(*right_type_decayed)) {
    auto cast_node = std::make_unique<TypeCastExpr>(
        node.releaseRight(), right_type_decayed->toString(),
        true, // implicit conversion
        node.getLocation());
    ...
    if (left_type->isPointer() || right_type->isPointer()) {
      if (op == "*") {
        ...
        error_msg = "Invalid operation: cannot use modulo with pointers ('" +
                    left_type->toString() + "' % '" + right_type->toString() +
                    "')";
      } else if (op == "-" && left_type->isPointer() &&
                 right_type->isPointer()) {
        error_msg = "Invalid operation: cannot subtract pointers of different "
                    "types ('" +
                    left_type->toString() + "' - '" + right_type->toString() +
                    "')";
      } else if (op == "+" && left_type->isPointer() &&
        ...
        error_msg = "Invalid operation: cannot add two pointers ('" +
                 ...
                 !right_type->isIntegral()) {
        error_msg = "Invalid operation: pointer arithmetic requires integer "
                    "operand ('" +
                    left_type->toString() + "' " + op + " '" +
                    right_type->toString() + "')";
      } else if ((op == "+" || op == "-") && right_type->isPointer() &&
                 !left_type->isIntegral()) {
        error_msg = "Invalid operation: pointer arithmetic requires integer "
                    "operand ('" +
                    left_type->toString() + "' " + op + " '" +
                    right_type->toString() + "')";
  ...
  // For arithmetic operators, use type promotion rules and insert conversion
  // nodes
  if (op == "+" || op == "-" || op == "*" || op == "/" || op == "%") {
    // USER STORY #11: Apply implicit conversions for arithmetic operations
    // Get the common type (applies integer promotion + usual arithmetic
    // conversions)
    ...
    if (common_type) {
      ...
      if (!left_type->equals(*common_type)) {
        auto cast_node = std::make_unique<TypeCastExpr>(
            node.releaseLeft(), common_type->toString(),
            true, // implicit conversion
            node.getLocation());
      ...
      if (!right_type->equals(*common_type)) {
        auto cast_node = std::make_unique<TypeCastExpr>(
            node.releaseRight(), common_type->toString(),
            true, // implicit conversion
            node.getLocation());
        set_expression_type(cast_node.get(), common_type);
        node.setRight(std::move(cast_node));
  ...
  // For comparison operators, result is int (representing boolean in C)
    ...
    if (left_type->isArithmetic() && right_type->isArithmetic()) {
      ...
      if (common_type) {
        if (!left_type->equals(*common_type)) {
          auto cast_node = std::make_unique<TypeCastExpr>(
              node.releaseLeft(), common_type->toString(), true,
              node.getLocation());
          set_expression_type(cast_node.get(), common_type);
          node.setLeft(std::move(cast_node));
        }
        ...
        if (!right_type->equals(*common_type)) {
          auto cast_node = std::make_unique<TypeCastExpr>(
              node.releaseRight(), common_type->toString(), true,
              node.getLocation());
          set_expression_type(cast_node.get(), common_type);
          node.setRight(std::move(cast_node));
        }
  ...
  // For bitwise operators, result follows integer promotion
    ...
    if (!left_type->equals(*left_promoted)) {
      auto cast_node = std::make_unique<TypeCastExpr>(node.releaseLeft(),
    ...
    if (!right_type->equals(*right_promoted)) {
      auto cast_node = std::make_unique<TypeCastExpr>(
          node.releaseRight(), right_promoted->toString(), true,
          node.getLocation());
...
void SemanticAnalyzer::visit(UnaryExpr &node) {
  ...
  if (op == "&") {
    ...
    if (!is_lvalue(node.getOperand())) {
      add_error("Cannot take address of rvalue (non-lvalue expression)",
                node.getLocation());
      return;
    }
    ...
    std::shared_ptr<Type> result_type;
    if (operand_type->getBaseType() == Type::BaseType::STRUCT) {
      result_type = Type::makeStructPointer(
          operand_type->getStructName(), operand_type->getPointerDepth() + 1);
    } else {
      result_type = Type::makePointer(operand_type->getBaseType(),
                                      operand_type->getPointerDepth() + 1);
    }
    set_expression_type(&node, result_type);
    return;
  }
  ...
  if (op == "*") {
    ...
    if (!operand_type->isPointer()) {
      add_error("Cannot dereference non-pointer type '" +
    ...
    std::shared_ptr<Type> result_type;
    if (operand_type->getBaseType() == Type::BaseType::STRUCT) {
      int new_depth = operand_type->getPointerDepth() - 1;
      if (new_depth > 0) {
        result_type =
            Type::makeStructPointer(operand_type->getStructName(), new_depth);
      } else {
        // Dereferencing to the struct itself
        // Look up the full struct definition to ensure we have members
        auto it = struct_types_.find(operand_type->getStructName());
        if (it != struct_types_.end()) {
          result_type = it->second;
        } else {
          // Fallback if not found (should be rare if type checked)
          // Create a basic struct type with the name
          result_type = Type::makeStruct(operand_type->getStructName(), {});
        }
      }
    } else {
      result_type = Type::makePointer(operand_type->getBaseType(),
  ...
  if (op == "++" || op == "--") {
    ...
    if (!is_lvalue(node.getOperand())) {
      add_error("Increment/decrement operator requires an lvalue",
                node.getLocation());
      return;
    }
    ...
    if (!operand_type->isArithmetic() && !operand_type->isPointer()) {
      add_error(
          "Increment/decrement operator requires arithmetic or pointer type",
          node.getLocation());
      return;
    }
  ...
  if (op == "-" || op == "+") {
    ...
    if (!operand_type->isArithmetic()) {
      add_error("Unary " + op + " requires arithmetic type",
                node.getLocation());
      return;
    }
...
void SemanticAnalyzer::visit(CallExpr &node) {
  ...
  const IdentifierExpr *callee_id =
      dynamic_cast<const IdentifierExpr *>(node.getCallee());
  if (!callee_id) {
  ...
  if (!func_symbol_opt.has_value()) {
    // Function not declared - this will be caught by undeclared identifier
    // checking
    return;
  }
  ...
  if (!func_symbol.is_function) {
    add_error("'" + callee_id->getName() + "' is not a function",
              node.getLocation());
    return;
  }
...
void SemanticAnalyzer::visit(MemberAccessExpr &node) {
  ...
  if (is_arrow) {
    ...
    if (object_type->getPointerDepth() > 0) {
      ...
      auto pointed_type = Type::makePointer(object_type->getBaseType(),
...
std::shared_ptr<Type>
SemanticAnalyzer::apply_integer_promotion(std::shared_ptr<Type> type) const {
  return applyIntegerPromotion(type);
}

std::shared_ptr<Type>
SemanticAnalyzer::get_common_type(std::shared_ptr<Type> left,
                                  std::shared_ptr<Type> right) const {
  return getCommonArithmeticType(left, right);
}

std::shared_ptr<Type>
SemanticAnalyzer::needs_conversion(std::shared_ptr<Type> from,

...
Type::Type(BaseType base)
    : base_type_(base), pointer_depth_(0), is_array_(false), array_size_(0),
      struct_name_("") {}

Type::Type(BaseType base, int pointer_depth)
    : base_type_(base), pointer_depth_(pointer_depth), is_array_(false),
      array_size_(0), struct_name_("") {}

Type::Type(BaseType base, int array_size, bool is_array)
    : base_type_(base), pointer_depth_(0), is_array_(is_array),
      array_size_(array_size), struct_name_("") {}

Type::Type(BaseType base, int pointer_depth, int array_size)
    : base_type_(base), pointer_depth_(pointer_depth), is_array_(true),
      array_size_(array_size), struct_name_("") {}

Type::Type(const std::string &struct_name,
           const std::vector<StructMember> &members)
    : base_type_(BaseType::STRUCT), pointer_depth_(0), is_array_(false),
      array_size_(0), struct_name_(struct_name), struct_members_(members) {}
...
std::shared_ptr<Type> Type::makeStructPointer(const std::string &name,
                                              int depth) {
  auto type = std::make_shared<Type>(BaseType::STRUCT, depth);
...
bool Type::isIntegral() const {
  ...
  return base_type_ == BaseType::CHAR || base_type_ == BaseType::SHORT ||
         base_type_ == BaseType::INT || base_type_ == BaseType::LONG;
}
...
bool Type::isFloatingPoint() const {
  ...
  return base_type_ == BaseType::FLOAT || base_type_ == BaseType::DOUBLE;
}
...
std::shared_ptr<Type>
Type::getMemberType(const std::string &member_name) const {
  if (base_type_ != BaseType::STRUCT) {
    return nullptr;
  }
...
bool isValidBinaryOperator(const Type &left, const Type &right,
                           const std::string &op) {
  ...
  if (op == "<" || op == ">" || op == "<=" || op == ">=" || op == "==" ||
    ...
    // Can compare pointers of same type (strict equality for different pointer
    // types)
    if (left.isPointer() && right.isPointer()) {
...
std::shared_ptr<Type> getCommonArithmeticType(std::shared_ptr<Type> left,
  ...
  if (left_base == Type::BaseType::DOUBLE ||
      right_base == Type::BaseType::DOUBLE) {
    return Type::makeDouble();
  }

  // If either is float, convert to float
  if (left_base == Type::BaseType::FLOAT ||
      right_base == Type::BaseType::FLOAT) {
    return Type::makeFloat();
  }

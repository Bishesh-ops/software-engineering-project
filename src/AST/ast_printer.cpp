 ...
 * -------------------------------------------------------------
 * This class demonstrates how to implement the visitor pattern for AST
 * traversal. It prints a human-readable, indented representation of the AST for
 * debugging.
 *
 * Usage:
 *   ASTPrinter printer;
...
public:
  ...
  void visit(CallExpr &node) override {
    ...
    printIndent();
    std::cout << "Arguments (" << node.getArguments().size()
              << "):" << std::endl;
    increaseIndent();
  ...
  void visit(MemberAccessExpr &node) override {
    printIndent();
    std::cout << "MemberAccessExpr(." << node.getMemberName()
              << (node.getIsArrow() ? " (arrow)" : "") << ") "
              << "[" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
  ...
  void visit(CompoundStmt &node) override {
    printIndent();
    std::cout << "CompoundStmt (" << node.getStatements().size()
              << " statements) "
              << "[" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
  ...
  void visit(TypeCastExpr &node) override {
    printIndent();
    std::cout << "TypeCastExpr(" << node.getTargetType() << ") "
              << "[" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
  ...
  void visit(SizeOfExpr &node) override {
    ...
    std::cout << "SizeOfExpr(";
    if (node.isTypeSize()) {
      std::cout << "Type: " << node.getTargetType();
    } else {
      std::cout << "Expr";
    }
    std::cout << ") [" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
  ...
  void visit(TypeDecl &node) override {
    printIndent();
    std::cout << "TypeDecl(typedef " << node.getUnderlyingType() << " "
              << node.getName() << ") "
              << "[" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
  }
  ...
  void visit(StructDecl &node) override {
    printIndent();
    std::cout << "StructDecl(" << node.getName() << ", "
              << node.getFields().size() << " fields) "
              << "[" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
  ...
  void visit(FunctionDecl &node) override {
    printIndent();
    std::cout << "FunctionDecl(" << node.getReturnType() << " "
              << node.getName() << ") "
              << "[" << node.getLine() << ":" << node.getColumn() << "]"
              << std::endl;
    ...
    increaseIndent();
    printIndent();
    std::cout << "Parameters (" << node.getParameters().size()
              << "):" << std::endl;
    increaseIndent();
 ...
 * auto init = std::make_unique<LiteralExpr>("5",
 * LiteralExpr::LiteralType::INTEGER, SourceLocation("test.c", 1, 5)); auto
 * varDecl = std::make_unique<VarDecl>("x", "int", std::move(init),

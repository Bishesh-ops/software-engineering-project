...
CompilerDriver::CompilerDriver() : errorCount(0) {
...
std::string CompilerDriver::getTempFileName(const std::string &baseName,
...
bool CompilerDriver::runCommand(const std::string &command,
                                std::string &output) {
  if (options.verbose) {
...
bool CompilerDriver::writeAssemblyFile(const std::string &assembly,
...
bool CompilerDriver::assembleFile(const std::string &asmFile,
...
bool CompilerDriver::linkFiles(const std::vector<std::string> &objFiles,
                               const std::string &executable) {
  reportInfo("Linking: " + std::to_string(objFiles.size()) +
             " object file(s) -> " + executable);
...
bool CompilerDriver::compile(const std::string &sourceCode,
  ...
  if (!options.dumpTokensPath.empty()) {
    ...
    if (!outFile) {
      ...
      outFile << jsonOutput;
      outFile.close();
      reportInfo("  -> Token dump successful (" +
                 std::to_string(jsonOutput.size()) + " bytes)");
    }
  }
  ...
  reportInfo("  -> AST constructed successfully (" +
             std::to_string(ast.size()) + " declarations)");
  ...
  if (!options.dumpAstPath.empty()) {
    ...
    if (!outFile) {
      ...
      outFile << jsonOutput;
      outFile.close();
      reportInfo("  -> AST dump successful (" +
                 std::to_string(jsonOutput.size()) + " bytes)");
    }
  }
  ...
  for (const auto &decl : ast) {
    if (auto *funcDecl = dynamic_cast<FunctionDecl *>(decl.get())) {
      try {
        ...
        reportError("IR generation failed for function '" +
                    funcDecl->getName() + "': " + e.what());
        return false;
      }
    }
  }

  reportInfo("  -> Generated IR for " + std::to_string(irFunctions.size()) +
             " function(s)");
  ...
  reportInfo("  -> Generated " + std::to_string(assembly.size()) +
             " bytes of assembly");
  ...
  if (!options.dumpAsmPath.empty()) {
    ...
    if (!outFile) {
      ...
      outFile << assembly;
      outFile.close();
      reportInfo("  -> Assembly dump successful (" +
                 std::to_string(assembly.size()) + " bytes)");
    }
  }
  ...
  if (dumpOnlyMode) {
    reportInfo("========================================");
    reportInfo("Dump-only mode: skipping assembly and linking");
    reportInfo("All visualization dumps written successfully");
    reportInfo("========================================");
    return true;
  }
  ...
  if (!options.dumpHexPath.empty()) {
    ...
    if (hexOutput.empty()) {
      ...
      if (!outFile) {
        ...
        outFile << hexOutput;
        outFile.close();
        reportInfo("  -> Hex dump successful (" +
                   std::to_string(hexOutput.size()) + " bytes)");
      }
    }
  }
...
bool CompilerDriver::compileMultiple(
